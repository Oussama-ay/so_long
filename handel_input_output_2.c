...
/*   Updated: 2025/01/03 08:39:31 by oayyoub          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	stream(t_game *game, void **frames, int i, t_cord cord)
{
	if (!frames || !frames[i])
		return ;
	mlx_put_image_to_window(game->mlx, game->win, frames[i],
		cord.x * 64, cord.y * 64);
	delay_ms(90000000);
	i++;
}
...
void	draw_player(t_game *game, int x, int y)
{
	...
	if (game->player_direction == 1 && game->sprites.player_up)
		...
		stream(game, game->sprites.player_left,
			game->current_frame_player, cord);
	else if (game->player_direction == 3 && game->sprites.player_right)
		stream(game, game->sprites.player_right,
			game->current_frame_player, cord);
	else if (game->player_direction == 4 && game->sprites.player_down)
		stream(game, game->sprites.player_down,
			game->current_frame_player, cord);
}
...
void	draw_exit(t_game *game, int x, int y)
{
	...
	if (game->current_frame_exit < 4 && game->sprites.exit_animation)
		stream(game, game->sprites.exit_animation,
			game->current_frame_exit, cord);
	else if (game->sprites.exit_open)
		mlx_put_image_to_window(game->mlx, game->win,
			game->sprites.exit_open, x * 64, y * 64);
}
